...
function onSubmit(e){
    ...
    if(editingExpenseId === 0){
        axios.post('http://18.118.166.28:3000/home/expense', expenseObj, { headers: {'Authorization': token} })
        .then((response) => {
...
if (document.readyState == "loading" ){
    ...
    // axios.get('http://18.118.166.28:3000/home/show', { headers: {'Authorization': token} } )
...
function addNewLineElement(expenseDetails){
    ...
    delBtn.addEventListener('click', ()=> {
        ...
        axios.get(`http://18.118.166.28:3000/home/delete/${id}`, { headers: {'Authorization': token} })
        li.remove();
        }
    });
...
document.getElementById('premium').onclick = async function (e) {
    const response  = await axios.get('http://18.118.166.28:3000/purchase/premium-membership', { headers: {"Authorization" : token} });
    ...
    var options =
    {
     ...
     "handler": async function (response) {
        const result = await axios.post("http://18.118.166.28:3000/purchase/update-transaction-status", {
    ...
    rzrp1.on("payment.failed", () => {
        axios.post("http://18.118.166.28:3000/purchase/update-transaction-status", { order_id: response.data.order.id }, { headers: { "authorization": token } })
...
axios.get("http://18.118.166.28:3000/premium/show-leaderboard", { headers: { "authorization": token } })
    .then(res => {
...
let reportDisplayed = false;
    ...
    axios.get('http://18.118.166.28:3000/getAllUrl',{headers: {'Authorization' : token}})
    .then((res) => {
        if(res.status === 200){
            showUrls(res.data)
        }
...
async function getPageExpenses(page){
    ...
    let response = await axios.get(`http://18.118.166.28:3000/home/show/${page}`,{params:{perpage},headers: { "Authorization": token}} );
